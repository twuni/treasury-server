...
import org.twuni.money.treasury.responder.transaction.RefreshTransaction;

public class RefreshResponder implements Responder {

	private final Factory<Treasury> treasuryFactory;
	private final Adapter<Token, String> adapter = new TokenAdapter();
	private final Connection connection;

	public RefreshResponder( Factory<Treasury> treasuryFactory, Connection connection ) {
	...
	public Response respondTo( Request request ) {
		RefreshTransaction transaction = new RefreshTransaction( new String( request.getContent() ), treasuryFactory, adapter );
